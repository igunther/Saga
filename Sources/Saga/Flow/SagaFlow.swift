...
// MARK: - Overview
//
// OGFlow provides lightweight, type-safe coordinator/flow infrastructure for SwiftUI apps.
...
//                 own presentation modifiers (e.g. auto-height sheets from OGSheets).
//
...
public enum SagaPresentationStyle: Equatable {
    case sheet
    case fullScreen
}

public struct SagaPresentedItem: Identifiable, Equatable {
    public let id = UUID()
    public let style: SagaPresentationStyle
    public let view: AnyView
    public let isConfigured: Bool // Whether the view already has presentation modifiers

    public init(
        style: SagaPresentationStyle,
        view: AnyView,
    ...
    public static func == (lhs: SagaPresentedItem, rhs: SagaPresentedItem) -> Bool {
        lhs.id == rhs.id
    }
}
...
@MainActor
open class SagaRouter<Route: Hashable>: ObservableObject {
    @Published public var path: [Route] = []
    @Published public var sheet: SagaPresentedItem? = nil
    @Published public var fullScreen: SagaPresentedItem? = nil
    ...
    public func presentSheet(_ view: some View) {
        sheet = SagaPresentedItem(style: .sheet, view: AnyView(view))
    }

    /// Present a sheet that already has presentation configuration applied
    public func presentConfiguredSheet(_ view: some View) {
        sheet = SagaPresentedItem(style: .sheet, view: AnyView(view), isConfigured: true)
    }

    public func presentFullScreen(_ view: some View) {
        fullScreen = SagaPresentedItem(style: .fullScreen, view: AnyView(view))
    }
...
public enum SagaFlowResult<Value>: Sendable where Value: Sendable {
    case finished(Value)
    case cancelled
}
...
@MainActor
public final class SagaFlowHandle<Value: Sendable> {
    public var continuation: CheckedContinuation<SagaFlowResult<Value>, Never>?
...
@MainActor
public protocol SagaFlowable: ObservableObject {
    associatedtype Route: Hashable
    var router: SagaRouter<Route> { get }
}
